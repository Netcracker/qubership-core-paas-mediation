...
| GATEWAY_SYSTEM_TYPE | Enables [Gateway API](https://gateway-api.sigs.k8s.io/) routes ([HTTPRoute](https://gateway-api.sigs.k8s.io/reference/spec/#httproute), [GRPCRoute](https://gateway-api.sigs.k8s.io/reference/spec/#grpcroute)) observation if `gateway-api-default` is present. Can be `legacy-ingress` or `gateway-api-default` or `legacy-ingress,gateway-api-default`. Ensure PaaS Mediation Service has sufficient permissions to operate (`get`, `list`, `watch`) with `gateway.networking.k8s.io/v1` entities (`HTTPRoute`, `GRPCRoute`) | false   | No       |
